#Exploration of Red Wine Attributes by Raymond Atherley
========================================================
...
Althought the scoring is between 0 and 10, the range of quality ratings is between 3 and 8 for the wines in this dataset. We see, however, that the great majority of the ratings chosen by the experts are 5 and 6. 


--------------------------------------------------------------------------------
A subset of wines with high alcohol content was created to check if this has an effect on the quality rating. High content was choosen as wines with an alcohol content at and above the 90% quantile. This resulted as wines with an alcohol content of 12% or higher.
...
The main features are alcohol and quality rating. However, we will look at others to see the affect on the wine quality. We will use them to build a predictive model.
...
We calculated a linear model as well to verify three independent variables that significantly affect the quality rating. These are alcohol, sulphates, and volatile acidity.

#### Linear Model ( three significant variables)
...
Actually, the strongest relationship I found was alcohol content.
...
```
The bins include the upper value and is greater than the lower value.
...
ggplot(aes(x = citric.acid, y = fixed.acidity), data=rwine) + 
  geom_point(alpha=1/3) +
  geom_smooth(method='lm', fun.y = 'mean', aes(color=alcohol.buckets)) + 
  scale_y_continuous(breaks=seq(4,16,1)) + 
...
#### Linear Model (Five Most Significant Variables)

```{r echo=FALSE}

m1 <- lm(formula = quality ~ alcohol, data = rwine)
...
```{r}
r_coef3 <- sqrt(0.344)
r_coef3 <- round(r_coef3, 3)
print(paste("The correlation coefficient is: ", r_coef3))
```
...
A linear model was created with six independent variables to determine the quality rating. The independent variables were alcohol, citric acid, volatile acidity, sulphates, and fixed acidity. The adjusted coefficient of determination was 0.344 or 34.4%. Therefore, the calculated correlation coefficient which was 58.7%, which shows moderate correlation. The model consists of independent variables that significantly influence the quality rating received for the wine. 
A limitation is that the model does not involve all variables in the dataset. When an additional variable total Sulfur dioxide was added to the model the coefficient of determination did not change showing that the variable did not have an additional influence on the quality rating determined by the model. 
In addition, it is possible that not all of the variables influencing the quality of the wine like the  weather, soil, and region were not included in the dataset. 
...
The analysis had several seemingly influencing variables and there was some difficulty with deciding which ones to focus on and which to compare with each other. Using the correlation function and linear models created helped with these decisions.  This analysis would be enriched with variables like the weather, the region the grapes were grown or the wine was made, and the year in which the wine was made.
